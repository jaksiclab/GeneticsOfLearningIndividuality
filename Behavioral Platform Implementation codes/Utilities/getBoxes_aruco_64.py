...
while True:
    ...
    cv2.putText(
        temp,
        "Align the LCD with the grid then press Enter to continue",
        (50, 50),
...
while not boxes_filled():
    ...
    if len(corners) > 0:
        ...
        for markerCorner, markerID in zip(corners, ids):
            ...
            maze_num = markerID_to_mazeNum(markerID)

            if maze_num <= 64 and boxes[maze_num - 1] == [-999, -999, -999, -999] and (
                cX > x_min and cX < x_max and cY > y_min and cY < y_max
            ):
                if (maze_num - 1) % 16 / 16 < 0.5 and maze_num < 65:
                    boxes[maze_num - 1] = [cX - 29, cY - 70, w, h]
                    image = cv2.rectangle(
                        image,
                    ...
                    boxes[maze_num - 1] = [cX - 173, cY - 70, w, h]
                    image = cv2.rectangle(
                        image,
                        (boxes[maze_num - 1][0], boxes[maze_num - 1][1]),
                        (boxes[maze_num - 1][0] + w, boxes[maze_num - 1][1] + h),
        ...
        cv2.putText(
            temp,
            "Press Esc and re-run program if it is taking too long. ",
            (50, 50),
            cv2.FONT_HERSHEY_SIMPLEX,
            0.5,
            (0, 0, 255),
...
while True:
    ...
    if boxes_filled():
        ...
        cv2.putText(
            temp,
            "All boxes not acquired. Re-run program. ",
            (50, 50),
            cv2.FONT_HERSHEY_SIMPLEX,
            0.5,
            (0, 0, 255),
